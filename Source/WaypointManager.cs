...
namespace FinePrint
{
    public class WaypointManager : MonoBehaviour
    {
        ...
        private static Texture2D mTexCloud;
        private static Texture2D mTexTire;
        ...
        public WaypointManager()
        {
            ...
            mTexCloud = GameDatabase.Instance.GetTexture("FinePrint/Textures/cloud", false);
            mTexTire = GameDatabase.Instance.GetTexture("FinePrint/Textures/tire", false);
            ...
            if (mTexCloud == null)
            {
                mTexCloud = new Texture2D(16, 16);
                mTexCloud.SetPixels32(Enumerable.Repeat((Color32)Color.magenta, 16 * 16).ToArray());
                mTexCloud.Apply();
            }

            if (mTexTire == null)
            {
                mTexTire = new Texture2D(16, 16);
                mTexTire.SetPixels32(Enumerable.Repeat((Color32)Color.magenta, 16 * 16).ToArray());
                mTexTire.Apply();
            }
        }
        ...
        public void OnGUI()
        {
            if (MapView.MapIsEnabled)
            {
                ...
                foreach (Waypoint wp in waypoints)
                {
                    ...
                    {
                        case "cloud":
                            Graphics.DrawTexture(screenRect, mTexCloud, new Rect(0, 0, 1f, 1f), 0, 0, 0, 0, RandomColor(wp.seed, alpha));
                            break;
                        case "tire":
                            Graphics.DrawTexture(screenRect, mTexTire, new Rect(0, 0, 1f, 1f), 0, 0, 0, 0, RandomColor(wp.seed, alpha));
                            break;
                        default:
